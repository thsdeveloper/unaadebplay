...
const step2Schema = z
  .object({
    ...
    // Telefone é armazenado MASCARADO no form ((00) 00000-0000); valida pelos dígitos.
    phone: z
      .string()
      .min(1, 'Telefone é obrigatório')
      .refine((v) => { const d = v.replace(/\D/g, ''); return d.length === 10 || d.length === 11; }, 'Telefone inválido'),
    password: z
      .string()
    ...
    responsiblePhone: z
      .string()
      .optional()
      .refine((v) => { if (!v) return true; const d = v.replace(/\D/g, ''); return d.length === 10 || d.length === 11; }, 'Telefone inválido'),
  })
  ...
  const handleSubmit = useCallback(async (): Promise<boolean> => {
    ...
    try {
      ...
      const registrationData = {
        ...
        phone: digitsOnly(data.phone), // grava só dígitos no perfil
        birthdate: data.birthdate ? new Date(data.birthdate).toISOString() : undefined,
        gender: data.gender,
        sector: data.sector, // agora é o UUID do setor (SectorBeat mapeia título→id)
        avatar: avatarId,
        responsibleName: data.responsibleName,
        responsiblePhone: data.responsiblePhone ? digitsOnly(data.responsiblePhone) : undefined,
      };
