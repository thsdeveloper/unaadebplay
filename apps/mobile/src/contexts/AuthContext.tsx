...
export const AuthProvider: React.FC<Props> = ({ children }) => {
    ...
    const login = async (email: string, password: string, rememberMe: boolean = false) => {
        ...
        try {
            ...
            // Login inválido é uma condição ESPERADA (não é bug) → console.log evita o overlay
            // vermelho do LogBox no dev; o usuário é avisado pelo flash-message abaixo.
            console.log('[login] falhou:', error?.message ?? error);
            toast.error(error?.message || 'Erro ao fazer login. Verifique sua conexão.');
            // Re-lança para o handleSignIn NÃO seguir no caminho de sucesso (haptic de sucesso
            // + prompt de biometria) quando o login na verdade falhou.
            throw error;
        } finally {
            setLoading(false);
        }
    ...
    // Login "Entrar com a Apple" (nativo iOS). Espelha o fluxo do login por email.
    const loginWithApple = async () => {
        ...
        try {
            ...
            // Usuário fechou o diálogo da Apple → silencioso (não é erro).
            if (error?.code === 'ERR_REQUEST_CANCELED') return;
            console.log('[apple login] falhou:', error?.message ?? error);
            toast.error(error?.message || 'Não foi possível entrar com a Apple.');
        } finally {
            setLoading(false);
        }
    };
