...
class EventsService {
  ...
  async getEvents(filters?: EventFilters, forceRefresh = false): Promise<EventsTypes[]> {
    try {
      ...
      // A agenda mostra passado + futuro; sem limit a API corta em 50 (default).
  ...
  /** Eventos em que o usuário está inscrito (evento + id da inscrição p/ cancelar). */
  async getSubscribedEvents(userId: string): Promise<{ event: EventsTypes; subscriptionId: string }[]> {
    try {
      const subs = await this.getUserSubscriptions(userId);
      const subByEvent: Record<string, string> = {};
      for (const sRow of subs as any[]) {
        if (sRow?.event_id && !subByEvent[sRow.event_id]) subByEvent[sRow.event_id] = sRow.id;
      }
      const ids = Object.keys(subByEvent);
      ...
      return events
        .filter((e) => !!subByEvent[e.id])
        .map((event) => ({ event, subscriptionId: subByEvent[event.id] }));
    } catch (error) {
