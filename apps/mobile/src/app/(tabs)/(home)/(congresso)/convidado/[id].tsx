...
const ConvidadoPage = React.memo(() => {
  ...
  const roleLabel = (Array.isArray(role) ? role[0] : role)?.trim() || undefined;
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      try {
        // Perfil PÚBLICO via API (RPC get_public_profiles) — `profiles` é owner-only na RLS.
        const data = await api.users.get<Profile>(id as string);
      ...
      <ScrollView contentContainerStyle={[s.scroll, { paddingTop: insets.top + 64 }]} showsVerticalScrollIndicator={false}>
        ...
        <View style={s.headerBlock}>
          <View style={s.avatarRing}>
            <Avatar userId={profile.id} userAvatarID={profile.avatar ?? undefined} name={profile.first_name ?? undefined} size={120} />
          </View>
          <RNText style={s.name} numberOfLines={2}>{name}</RNText>
          {(roleLabel || profile.title) && (
            <View style={s.rolePill}>
              <RNText style={s.roleText}>{roleLabel || profile.title}</RNText>
            </View>
          )}
        </View>
