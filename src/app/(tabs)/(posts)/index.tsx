...
import { SlidersHorizontal, Newspaper, SearchX, RotateCw } from 'lucide-react-native';
...
/** Chip de categoria (horizontal) — dark, ativo em brand; ponto colorido opcional. */
const CategoryChip: React.FC<{ label: string; color?: string; active: boolean; onPress: () => void }> = ({ label, color, active, onPress }) => (
  <Pressable onPress={onPress} style={[s.chip, active && s.chipActive]} accessibilityRole="button" accessibilityLabel={label}>
    {!!color && !active && <View style={[s.chipDot, { backgroundColor: color }]} />}
  ...
  return (
    <View style={s.listHeader}>
      <View style={s.searchRow}>
        <View style={{ flex: 1 }}><EventSearchBar onSearch={onSearch} placeholder="Buscar notícias..." /></View>
        <Pressable onPress={onOpenFilters} style={s.filterBtn} accessibilityRole="button" accessibilityLabel="Filtrar notícias">
          <GlassSurface style={s.fill} glassEffectStyle="regular" pointerEvents="none" />
          <SlidersHorizontal size={20} color={SHEET.textPrimary} />
          {filterCount > 0 && <View style={s.filterDot} />}
        </Pressable>
      </View>
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={s.chipsRow} keyboardShouldPersistTaps="handled">
        <CategoryChip label="Todas" active={!activeCategory} onPress={() => onSelectCategory(null)} />
        {categories.map((cat) => (
          <CategoryChip key={cat.id} label={cat.name} color={cat.color} active={activeCategory === cat.id} onPress={() => onSelectCategory(cat.id)} />
        ))}
      </ScrollView>
    </View>
  );
